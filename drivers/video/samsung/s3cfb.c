...
static int s3cfb_map_video_memory(struct fb_info *fb)
{
	...
	if (pdata && pdata->pmem_start[win->id] && (pdata->pmem_size[win->id] >= fix->smem_len)) {
		...
		num_pages = fix->smem_len >> PAGE_SHIFT;
		pages = vmalloc(num_pages * sizeof(struct page *));

		for (i = 0; i < num_pages; i++)
			pages[i] = pfn_to_page((pdata->pmem_start[win->id] >> PAGE_SHIFT) + i);
		printk("win->id=%d, pmem_start=0x%x\n",win->id, pdata->pmem_start[win->id]);

		fb->screen_base = vmap(pages, num_pages, VM_MAP,
	...
	if (!fb->screen_base)
	{
		printk("DDDDDDDDDDDDDDDDDMMMMMMMMMMMMMMMMMMMAAAAAAAAAAAAAAAAA\n");
		return -ENOMEM;
	}

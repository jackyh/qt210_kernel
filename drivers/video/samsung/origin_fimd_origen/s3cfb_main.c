				...
				struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < pdata->nr_wins; i++) {
		...
		snprintf(temp, sizeof(temp), "[fb%d] %s\n",
		...
		for (j = 0; j < pdata->nr_wins; j++) {
			...
			if (fb) {
				...
				if (win->id == pdata->default_win)
					s3cfb_unmap_default_video_memory
					    (fbdev[i], fb);
				else
...
{
	struct s3cfb_global *info = container_of(h,  \
				struct s3cfb_global, early_suspend);
	struct s3c_platform_fb *pdata = to_fb_plat(info->dev);
	struct platform_device *pdev = to_platform_device(info->dev);
	struct s3cfb_global *fbdev[2];
	int i;

	printk(KERN_INFO"s3cfb_early_suspend is called\n");
...
#if defined(CONFIG_FB_S3C_AMS369FG06)
...
void s3cfb_late_resume(struct early_suspend *h)
{
	...
	printk(KERN_INFO "s3cfb_late_resume is called\n");

...
#define DAI_MANAGER	(1 << 1) /* Dai is the manager */
	...
	u32	suspend_i2scon;
	u32	suspend_i2spsr;
	u32	suspend_ass_clk_src;
	u32	suspend_ass_clk_div;
	u32	suspend_ass_clk_gate;
};

static int clk_enabled;
struct clk *mout_audss = NULL;
struct clk *iis_clk, *iis_busclk, *iis_ipclk; /* these belong shomewhere else */
	  ...
	  int clk_id, unsigned int rfs, int dir)
{
	...
	case SAMSUNG_I2S_RCLKSRC_1: /* clock corrsponding to IISMOD[10] := 1 */
		if ((i2s->quirks & QUIRK_NO_MUXPSR)
			...
			clk_id = 1;
#if 0 //todo.... commented temporarily...need to check exact functionality 
		if (!any_active(i2s)) {
			if (i2s->op_clk) {
	...
	mod &= ~(1<<12);//todo... need to add following proper switch case
	mod &= ~(MOD_OPCLK_MASK);
	mod |= MOD_OPCLK_PCLK;  
	writel(mod, i2s->addr + I2SMOD);
...
//	mod |= tmp; //todo... commented temporary
	writel(mod, i2s->addr + I2SMOD);
	...
	struct snd_soc_dai *dai)
{
	...
	if (!is_opened(other))
		i2s_set_sysclk(dai, SAMSUNG_I2S_CDCLK,
				0, SND_SOC_CLOCK_IN);
	if(clk_enabled){
         clk_disable(iis_busclk);
         clk_disable(iis_clk);
         clk_disable(iis_ipclk);
         clk_disable(i2s->clk);
         clk_enabled = 0;
        }
...
static int config_setup(struct i2s_dai *i2s)
{
	...
	set_bfs(i2s, bfs);
	set_rfs(i2s, rfs);
#if 0 //todo...need to check functionality
	if (!(i2s->quirks & QUIRK_NO_MUXPSR)) {
...
#endif
		psr = 4;//todo.... need to set psr value according to clock not hardcode
		writel((readl(i2s->addr + I2SPSR)&(~(0x3F << 8))),i2s->addr + I2SPSR);
		writel(((psr - 1) << 8) | PSR_PSREN, i2s->addr + I2SPSR);
		//printk("RCLK_SRC=%luHz PSR=%u, RCLK=%dfs, BCLK=%dfs\n",__func__,
		//		i2s->rclk_srcrate, psr, rfs, bfs);
...
static int i2s_suspend(struct snd_soc_dai *dai)
{
	...
	//if (dai->active) {
		...
		i2s->suspend_ass_clk_src = readl(S5P_CLKSRC_AUDSS);
		i2s->suspend_ass_clk_div = readl(S5P_CLKDIV_AUDSS);
		i2s->suspend_ass_clk_gate = readl(S5P_CLKGATE_AUDSS);
//	}
...
//	if (dai->active) {
		...
		writel(i2s->suspend_ass_clk_src, S5P_CLKSRC_AUDSS);
		writel(i2s->suspend_ass_clk_div, S5P_CLKDIV_AUDSS);
		writel(i2s->suspend_ass_clk_gate,S5P_CLKGATE_AUDSS);
//	}
...
static int samsung_i2s_dai_probe(struct snd_soc_dai *dai)
{
	...
	clk_enable(i2s->clk);
#if 0 //todo ...power domain need to be enabled later
